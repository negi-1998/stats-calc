...
void __fastcall TmymeanForm::ungroupedOKButtonClick(TObject *Sender)
{
	...
	int decimalCount=0;
	if(AnsiData[AnsiData.Length()-1]=='.'){
		ungroupedErrorLabel->Text = "Number should have something after the decimal";
		return;
	}
	for(int i=0; i<AnsiData.Length(); i++){

	}
	float floatData = StrToFloat(AnsiData);
	ungroupedData.push_back(floatData);
	ungroupedEdit->Text = "";
    ungroupedErrorLabel->Text = "";
}
//---------------------------------------------------------------------------
